...
conn = mcdb.connect(host="localhost", user="root", passwd="Mysql221b@", database='myproject1')
...
def Useraddprocess(request):
    if request.method == 'POST':
        ...
        try:
            ...
            f = open("/static/upload/"+photo,'wb')
            for i in photo:
                f.write(i)
            f.close()
            id_proof = request.FILES['id_proof']
            fi = open("/static/upload/"+id_proof.name, 'wb')
            for i in idproof:
                fi.write(i)
            fi.close()
        except:
...
def Userdelete(request,id):
    ...
    cur.execute("delete from `payment_master` where `Membership_Id` = {}".format(id))
    cur.execute("delete from `membership_master` where `User_Id` = {}".format(id))
...
def MembershipEditProcess(request,id):
    if request.method == 'POST':
        ...
        cur.execute("update `payment_master` set `Payment_status` ='{}' where `Membership_Id`='{}'".format(m_status,id))
        conn.commit()
